...
if nav_option == "项目介绍":
    ...
    # 项目概述：分两列布局（文字+图片）
    c1, c2 = st.columns([2, 1])  # 第一列宽度是第二列的2倍
    with c1:
        st.header("📋 项目概述")  # 二级标题
        st.markdown("""
        ...
        """)
        # 项目主要特点
        st.markdown('<h4>主要特点：</h4>', unsafe_allow_html=True)
    ...
    with c3:
        st.markdown('<h4>🎯 目标一：</h4>', unsafe_allow_html=True)
        st.markdown('''
            分析影响因素
            - 识别关键学习指标
            - 探索成绩相关因素
        ...
        ''')
    with c4:
        st.markdown('<h4>🎯 目标二：</h4>', unsafe_allow_html=True)
        st.markdown('''
            可视化展示
            - 专业对比分析
            - 性别差异研究
        ...
        ''')
    with c5:
        st.markdown('<h4>🎯 目标三：</h4>', unsafe_allow_html=True)
        st.markdown('''
            成绩预测
            - 机器学习模型
            - 个性化预测
